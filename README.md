...
2. [한눈에 보기](#2-한눈에-보기)
3. [빠른 실행](#3-빠른-실행)
4. [아키텍처 — 3레이어 클린 구조](#4-아키텍처--3레이어-클린-구조)
5. [디렉토리 구조](#5-디렉토리-구조)
6. [파이프라인 18단계 상세](#6-파이프라인-18단계-상세)
7. [수집 (Collector)](#7-수집-collector)
8. [스코어링 알고리즘](#8-스코어링-알고리즘)
9. [정기공고 탐지](#9-정기공고-탐지)
10. [L3 강공고 정책](#10-l3-강공고-정책)
11. [수주 예측 (Win Prediction)](#11-수주-예측-win-prediction)
12. [BD 마일스톤 자동 배정](#12-bd-마일스톤-자동-배정)
13. [설정 파일 (configs/)](#13-설정-파일-configs)
14. [Google Sheets 10시트 구조](#14-google-sheets-10시트-구조)
15. [새 컬렉터 추가 방법](#15-새-컬렉터-추가-방법)
16. [테스트](#16-테스트)
17. [핵심 원칙](#17-핵심-원칙)
18. [BD Intelligence 웹 플랫폼](#18-bd-intelligence-웹-플랫폼)
19. [ML 수주예측 학습 파이프라인](#19-ml-수주예측-학습-파이프라인)
20. [Streamlit 팀 배포 앱](#20-streamlit-팀-배포-앱)
...
**🎯 영업 파이프라인 기반 스코어링 v6 + 비사업성 공고 3단계 필터 + 선정 데이터 ML 학습 + BD 플랫폼 Phase 1**

**스코어링 v6 — 실제 파이프라인 데이터 기반 전면 정교화:**

| 항목 | 내용 |
|------|------|
| **코어 키워드** | ax sprint, ax-sprint300, 지능화경영, 생산운영관리, 설비품질관리, 압입, 프레스가공, 사출, 다이캐스팅, 절삭가공, apc, cpv, sop 추가 |
| **솔루션맵 확장** | RecipeAI(압입조건/혼련/apc/cpv/sop), InspectionAI(사양식별/전달오차/불량검출), GenAI(지능화경영/생산운영관리), InfraDS(운영제어/npu/엣지디바이스), ManufacturingDT(축소셀라인/자율제조라인/통합관제시스템) |
| **가점 키워드** | 42+ 신규 — ax sprint(6), 지능화경영(5), 자동차부품(3), 조선해양(3), 공작기계(3), 불량검출(5), 혼련최적화(5) 등 |
| **콤보 키워드** | 24쌍 신규 — (ax,sprint,6), (지능화경영,agent,6), (자동차,ai,4), (조선,스케줄링,5), (산단,ax,5), (첨단제조,ai,5) 등 |
| **감점 수정** | "제약" 감점 제거 (종근당/동국제약 실고객), 컨설팅(5)/역량강화(4)/역량진단(4) MEDIUM 추가 |

**비사업성 공고 3단계 방어:**

| 단계 | 내용 | 효과 |
|------|------|------|
| **1단계: 제목 블랙리스트** | 20+ 키워드 (컨설팅 지원, 무료 교육, 참가자 모집, ksp, 국제협력 등) → 강제 D등급 | 교육/컨설팅 공고 즉시 차단 |
| **2단계: 감점 키워드** | 컨설팅, 역량강화 등 MEDIUM 감점 | fitness 감소로 등급 하락 |
| **3단계: 본문 체크** | body_text < 50자인 A등급 → B등급 하향 | 제목만으로 A등급 방지 |

**선정 데이터 기반 ML 학습:**
- 실제 선정 19건 학습 데이터 저장 (`data/training/won_projects.jsonl`)
- 선정 사업 패턴: 제조AI특화(3), 자율형공장(3), 정부일반형(2), AI응용제품(1), 산업문제해결형AIAgent(1) 등
- 탈락 데이터는 내부/외부 요인 구분 불가 → 스코어링 미반영

**BD 플랫폼 Phase 1 — 공고 상세 페이지:**

| 기능 | 내용 |
|------|------|
| **5대 스코어 카드** | 수주확률 / 적합도 / 우선순위 / 산업점수 / 예산 |
| **기본정보 + BD 인텔리전스** | 주관기관, 부처, 마감일, L3, 정기공고, 파트너 후보, 마일스톤 |
| **솔루션 매칭 바** | 8개 솔루션별 점수 바 차트 |
| **키워드 태그** | 매칭 키워드 + 콤보 키워드 태그 클라우드 |
| **구조화 본문** | 사업목적/지원내용/지원대상/신청방법 구조화 표시 + 본문 전체 보기 |
| **AI 심층 분석** | Gemini 기반 적합도/제안전략/솔루션매핑/리스크 분석 |

**수집 사이트 조정:**
- ptp(포항TP), gdtp(경기대진TP) 비활성화 — 공고가 아닌 메뉴/네비게이션 수집 문제
- Cloud 상시 **19개** + 로컬 전용 **4개** (kiat, iitp, smart_factory, jbtp)

**InterX 타겟 업종 (파이프라인 분석 기반):**
1. 자동차부품 (최다), 2. 반도체/PCB, 3. 가전, 4. 조선, 5. 식품, 6. 제약, 7. 공작기계, 8. 철강
...
- **P3 네거티브 3단계**: Strong(92개/×6.0) + Medium(60개/×4.0) + Weak(12개/×2.0)
...
```
                          ┌─────────────────────────────────────┐
                          │     정부지원사업 공고 사이트 (20+)     │
                          │  bizinfo / NIPA / KISED / KOIIA ... │
                          └──────────────┬──────────────────────┘
                                         │
                          ┌──────────────▼──────────────────────┐
                          │         1. 수집 (Collect)            │
                          │  requests 병렬 크롤링 + Playwright   │
                          │  목록 페이지 → 상세 페이지 보강       │
                          │  본문 8000자 + 첨부파일 OCR (HWP/PDF)│
                          └──────────────┬──────────────────────┘
                                         │
                 ┌───────────────────────▼───────────────────────┐
                 │              2. 전처리 (Preprocessing)          │
                 │  notice_id 중복 제거 → 마감 공고 필터          │
                 │  TF-IDF cross-site 유사도 중복 제거 (0.82)    │
                 │  body_text → 구조화 섹션 추출                  │
                 │  (사업목적 / 지원내용 / 지원대상 / 예산 / 신청방법)│
                 └───────────────────────┬───────────────────────┘
                                         │
          ┌──────────────────────────────▼──────────────────────────────┐
          │                     3. 스코어링 (Scoring)                    │
          ...
          │  (body_text는 코어 키워드 체크에만 사용 — 오염 방지)         │
          │                                                             │
          │  ┌─────────────┐  ┌──────────────┐  ┌─────────────────┐    │
          │  │ 코어 키워드  │  │ 솔루션 매칭   │  │ 콤보 키워드 80쌍│    │
          │  │ 체크 (게이트)│  │ 8개 솔루션별  │  │ 동시출현 가점   │    │
          │  │ 없으면 → D급 │  │ 개별 점수     │  │ (+3 ~ +8점)    │    │
          │  └──────┬──────┘  └──────┬───────┘  └────────┬────────┘    │
          │         │                │                    │             │
          │         ▼                ▼                    ▼             │
          │  fitness = (positive × 5.0) - (negative × 6.0/4.0/2.0)    │
          │  industry = 비0점 솔루션 평균                               │
          │  priority = fitness × 0.6 + industry × 0.4                 │
          │                                                             │
          │  등급: A(>=48) / B(>=30) / C(>=18) / D(<18)                │
          └──────────────────────────────┬──────────────────────────────┘
                                         │
          ┌──────────────────────────────▼──────────────────────────────┐
          │                    4. 부가 분석 (Analysis)                   │
          │                                                             │
          │  ┌──────────┐ ┌───────────┐ ┌──────────┐ ┌──────────────┐ │
          │  │ 정기공고  │ │ 수주 예측  │ │ L3 강공고│ │ 클러스터링   │ │
          │  │ 탐지      │ │ Rule + ML │ │ 필터링   │ │ TF-IDF      │ │
          │  │ 18그룹    │ │ 12피처    │ │          │ │              │ │
          │  │ 120+패턴  │ │ GBM 98.5% │ │         │ │              │ │
          │  └──────────┘ └───────────┘ └──────────┘ └──────────────┘ │
          │                                                             │
          ...
          │                                                             │
          │  ┌────────────┐ ┌──────────┐ ┌───────────┐ ┌────────────┐ │
          │  │ Google     │ │ SQLite   │ │ Streamlit │ │ Slack /    │ │
          │  │ Sheets     │ │ DB 저장  │ │ 대시보드  │ │ Telegram   │ │
          │  │ 10시트     │ │ + 이력   │ │ 실시간    │ │ 알림       │ │
          │  └────────────┘ └──────────┘ └───────────┘ └────────────┘ │
          │  ┌────────────┐ ┌──────────┐                              │
          │  │ CSV        │ │ ML 학습  │                              │
...
| 원칙 | 구현 |
|------|------|
| **Clean Architecture** | core(도메인) → application(유스케이스) → infrastructure(외부) 단방향 의존 |
| **무료 운영** | Gemini 2.0 Flash 무료 API + scikit-learn + Streamlit Cloud |
| **설정 분리** | 모든 파라미터는 `configs/*.yaml`에 — 코드 하드코딩 금지 |
| **점진적 ML** | Rule-based 시작 → 50건+ 축적 시 GradientBoosting 자동 전환 |
| **장애 격리** | 수집기 1개 실패해도 나머지 정상 진행 (try/except per site) |
| **OCR 보강** | body_text 부족 시 첨부파일(HWP/PDF) 자동 다운로드 → 텍스트 추출 |

### 수주 예측 모델 (Win Prediction)
      ...
      └─ 3겹 필터 (기본 → L3 → 키워드)
          └─ 스코어링 (fitness → priority → grade A/B/C/D)
              └─ 부가 분석 (정기공고 탐지 / win_probability / 클러스터링)
                  └─ Google Sheets 10시트 자동 업로드
                      └─ Slack/Telegram 알림
```

| 항목 | 수치 |
|------|------|
| 수집 사이트 | Cloud 19개 + 로컬 전용 4개 = 23개 (requests + Playwright) |
| 파이프라인 단계 | 18단계 (수집~AI브리핑) |
| 유즈케이스 | 25개 (스코어링, 중복제거, 변경감지, ML예측, 유사공고, 스마트알림, 마일스톤 배정 등) |
| 정기공고 패턴 | 18그룹, 120+ aliases (priority 1/2/3 등급) |
| 콤보 키워드 | 80쌍 (두 키워드 동시 출현 시 가점) |
| 감점 분류 | 3단계 — Strong 92개(×6.0) / Medium 60개(×4.0) / Weak 12개(×2.0) |
| 등급 | A / B / C / D |
| 솔루션 프로필 | 8개 (ManufacturingDT ~ PdM) |
| 수주 예측 | Rule 가중합 + ML(GradientBoosting) 자동 전환 |
| Sheets 시트 수 | 10개 (97_상태변경로그 포함) |
| 웹 플랫폼 | FastAPI + Tailwind (4페이지 + 6 API) |
| REST API | FastAPI 6 엔드포인트 (Swagger UI 자동 생성) |
| 팀 배포 앱 | Streamlit Cloud (14개 탭, 무료 호스팅) |
| AI Agent | Gemini 무료 (공고분석 + 챗봇 + 브리핑) |
| 단위 테스트 | 357건 전체 통과 |
...
`run_engine.py` → `DailyPipelineOrchestrator.run()` 순으로 아래 18단계 실행.
...
[7]  BD 마일스톤 자동 배정
     └─ assign_milestone.py — L3강공고·D-day·등급 기준으로 M01/M05/P01 자동 배정
        M01: L3강공고=Y (기본), M05: L3+D-day≤14, P01: 파트너후보=Y

[7B] 상태변경 로그
     └─ 검토상태/BD마일스톤 변경 이력 자동 기록 → 97_상태변경로그 시트

[8]  정기공고 탐지
     └─ recurring.yaml 패턴 매칭 → recurring_flag/group 설정
...
[18] AI 일일 브리핑 (Gemini)
     └─ 등급별/긴급/L3/정기공고 요약 → Slack/Telegram 전달
```
...
### 8-2B. v3 고도화 — 6가지 추가 분석 (ScoreCard 확장 필드)
...
### 8-3. 솔루션별 키워드 맵 (SOLUTION_MAP 요약)
...
### 10-2. recurring.yaml 주요 패턴 (18그룹, 120+ aliases)
...
`assign_milestone.py` — L3 강공고·D-day·등급 기준으로 BD 마일스톤 자동 배정.
...
| `20_요약대시보드` | 보고용 요약 | 실행일·총수집·등급별 건수·사이트별 현황 |
| `22_KPI현황` | 실행별 성능 KPI | 실행ID·소요시간·수집수·A등급수·L3수 |
| `93_사이트별수집통계` | 사이트별 수집 집계 | (시장 분석 참고용) |
| `94_실행로그` | 파이프라인 실행 이력 | 실행ID·시작/종료시각·상태 |
| `96_수집에러로그` | 수집 오류 사이트 | 사이트코드·오류메시지·발생시각 |
...
| **📈 시각화** | 종합 시각화 | 등급 분포 + 사이트별 통계 + 트렌드 차트 |
| **🔥 A 등급 상세** | A등급 집중 | A등급 공고 상세 분석 + 추천 액션 |
| **⚠️ 긴급 마감** | 긴급 관리 | D-3/D-7 마감 임박 공고 + 알림 |
| **📊 수집 트렌드** | 트렌드 분석 | 수집 추이 + 기간별 공고 변화 |
| **💰 예산 분석** | 예산 현황 | 예산 규모별 분포 + 예산 트렌드 |
| **🏅 사이트 품질** | 품질 등급 | 사이트별 수집 성공률 + 데이터 품질 A~D |
| **📎 사업공고 첨부** | 첨부 분석 | 첨부파일 목록 + OCR 파싱 결과 |
| **🧠 포트폴리오 분석** | 종합 분석 | 등급 분포 + 키워드 트렌드 + 시장 동향 |
| **📝 CRM 메모** | 메모 관리 | 공고별 메모 작성·편집 |
| **📅 마감 캘린더** | 마감 관리 | D-3/D-7/D-30 KPI + 타임라인 차트 + 긴급 리스트 |
| **🎯 BD 파이프라인** | BD 현황 | 마일스톤별 공고 현황 + 진행 상태 |
| **📤 보고서 다운로드** | 보고서 | Excel/CSV/PDF 보고서 다운로드 |
| **🤖 ML 수주예측** | 수주 확률 | 공고별 수주 확률 분포 + 유망 TOP10 |

### 20-3. UI 디자인 — interxlab.com 공식 팔레트
...
├── streamlit_app.py          # Enterprise Dashboard v7 (11탭, ~2500줄)
