...
5. [디렉토리 구조](#5-디렉토리-구조)
6. [파이프라인 17단계 상세](#6-파이프라인-17단계-상세)
...
12. [담당자 자동 배정 & BD 마일스톤](#12-담당자-자동-배정--bd-마일스톤)
...
19. [경쟁사 분석 리포트](#19-경쟁사-분석-리포트)
20. [제안서 자동 생성 v2](#20-제안서-자동-생성-v2)
21. [ML 수주예측 학습 파이프라인](#21-ml-수주예측-학습-파이프라인)
22. [Streamlit 팀 배포 앱](#22-streamlit-팀-배포-앱)
...
```
          ...
          │                                                             │
          │  ┌──────────┐ ┌───────────┐ ┌──────────┐ ┌──────────────┐ │
          │  │ 정기공고  │ │ 수주 예측  │ │ 경쟁사   │ │ 담당자 배정  │ │
          │  │ 탐지      │ │ Rule + ML │ │ 트래킹   │ │ + 마일스톤   │ │
          │  │ 18그룹    │ │ 12피처    │ │ 23사     │ │ M01/M05/P01 │ │
          │  │ 120+패턴  │ │ GBM 98.5% │ │ 2-tier  │ │              │ │
          │  └──────────┘ └───────────┘ └──────────┘ └──────────────┘ │
          │                                                             │
          │  ┌──────────┐ ┌───────────┐ ┌──────────┐ ┌──────────────┐ │
          │  │ L3 강공고 │ │ 클러스터링 │ │ 변경감지 │ │ 포트폴리오   │ │
          │  │ 필터링    │ │ TF-IDF    │ │ v2 해시  │ │ 분석 pandas │ │
          │  └──────────┘ └───────────┘ └──────────┘ └──────────────┘ │
          └──────────────────────────────┬──────────────────────────────┘
                                         │
          ┌──────────────────────────────▼──────────────────────────────┐
          ...
          │                                                             │
          │  ┌────────────┐ ┌──────────┐ ┌────────────┐ ┌───────────┐ │
          │  │ Google     │ │ SQLite   │ │ 제안서     │ │ Streamlit │ │
          │  │ Sheets     │ │ DB 저장  │ │ DOCX 생성  │ │ 대시보드  │ │
          │  │ 10시트     │ │ + 이력   │ │ A/B등급    │ │ 실시간    │ │
          │  └────────────┘ └──────────┘ └────────────┘ └───────────┘ │
          │  ┌────────────┐ ┌──────────┐ ┌────────────┐               │
          │  │ Slack /    │ │ CSV      │ │ ML 학습    │               │
...
```
  ...
  │      │ 없음               │
  │      ▼                    │
  │  레지스트리 매칭?         │──→ 부처/기관 키워드 매칭
  │  (산자부/TIPA/AX Sprint)  │    (산자부 7섹션, TIPA 4섹션)
  │      │ 없음               │
  │      ▼                    │
  │  AI 공고 분석             │──→ Gemini가 공고 본문에서 섹션 추출
  │      │ 실패               │
  │      ▼                    │
  │  기본 템플릿 (6섹션)      │
  └───────────┬──────────────┘
...
| 파이프라인 단계 | 17단계 (AI 브리핑 포함 시 18단계) |
| 유즈케이스 | 23개 (스코어링, 중복제거, 변경감지, 제안서, ML예측, 유사공고, 스마트알림 등) |
