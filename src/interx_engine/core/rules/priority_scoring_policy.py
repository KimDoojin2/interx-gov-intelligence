...
CORE_KEYWORDS = {
    ...
    # NOTE v5.7: 범용 단어("ai","품질","공정" 등)는 POSITIVE_KEYWORDS에서 가점만 부여
    # ── v6: 실제 영업 파이프라인 기반 추가 ──
    "ax sprint", "ax-sprint300", "ax sprint300",
    "지능화경영", "생산운영관리", "설비품질관리",
    "압입", "프레스가공", "사출", "다이캐스팅", "절삭가공",
    "apc", "cpv", "sop",
}
...
SOLUTION_MAP: Dict[str, Dict[str, float]] = {
    ...
    "InspectionAI": {
        ...
        "inspection.ai": 5,
        # v6: 실제 과제 — 헤드레스트, 기어전달오차, 디스플레이 품질
        "사양식별": 4, "자동판별": 4, "전달오차": 4,
        "불량검출": 5, "불량 검출": 5,
    },
    "SafetyAI": {
    ...
    "GenAI": {
        ...
        "copilot": 3, "코파일럿": 3,
        "제조암묵지": 4, "암묵지": 3,
        # v6: 실제 파이프라인 — 지능화경영, AI Agent 기반 생산/품질
        "지능화경영": 5, "지능화 경영": 5,
        "생산운영관리": 4, "생산운영": 3,
        "설비품질관리": 4, "설비품질": 3,
    },
    "InfraDS": {
        ...
        # v6: 실제 과제 — 운영제어연계, NPU, 엣지디바이스
        "운영제어": 4, "운영 제어": 4, "운영제어연계": 5,
        "npu": 3, "엣지디바이스": 4, "엣지 디바이스": 4,
    },
    "PdM": {
        "예지보전": 4, "설비": 3, "모니터링": 3,
...
POSITIVE_KEYWORDS: Dict[str, float] = {
    ...
    # ── v6: 실제 영업 파이프라인 기반 가점 ──
    # 실제 수주 사업 유형
    "ax sprint": 6, "ax-sprint300": 6, "ax sprint300": 6,
    "지능화경영": 5, "지능화 경영": 5,
    "생산운영관리": 4, "설비품질관리": 4,
    "해결형": 4, "지역형": 4,
    # 실제 과제 기술 키워드
    "압입조건": 4, "압입 조건": 4, "혼련최적화": 5, "혼련": 4,
    "공정스케줄링": 5, "블록배치": 4, "블록 배치": 4,
    "불량검출": 5, "불량 검출": 5,
    "apc": 4, "cpv": 4, "sop기반": 4,
    "수율상승": 4, "수율 상승": 4,
    "사양식별": 4, "전달오차": 4, "기어전달": 4,
    "운영제어연계": 5, "운영 제어": 4,
    "npu": 3, "엣지디바이스": 4, "엣지 디바이스": 4,
    # 실제 타겟 업종 (파이프라인 top 업종 — 가중치 2~3)
    "자동차부품": 3, "자동차 부품": 3,
    "반도체": 2, "pcb": 2,
    "조선": 2, "조선해양": 3,
    "가전": 2, "전장": 2,
    ...
    "트랙터": 2, "철강": 2,
    # 실제 참여 사업 프레임워크
    "컨소시엄": 2, "공급기업": 3, "주관기관": 3,
    "도입기업": 3, "수요기업모집": 0,  # 수요기업 모집은 블랙리스트
    "기획기관": 2, "테크노파크": 2,
    "산단": 3, "산업단지": 3, "국가산단": 3,
    "첨단제조": 4, "첨단제조플랫폼": 5,
}
...
NEGATIVE_KEYWORDS_MEDIUM: Dict[str, float] = {
    ...
    "컨설팅지원기관": 5,
    "컨설팅": 5, "컨설팅지원": 6, "컨설팅 지원": 6,
    "역량강화": 4, "역량진단": 4,
    # 산업보안 (제조AI와 다름)
    "산업보안": 5, "보안진단": 5, "사이버보안": 5, "정보보호": 4,
    # 지식재산/IP (특허 컨설팅)
    "지식재산": 5, "특허": 4, "ip투자": 5,
}
...
_TITLE_BLOCK_KEYWORDS = {
    "수요기업",   # 수요기업 모집 = InterX는 공급자, 대상 아님
    "수혜기업 모집",  # 수혜기업 모집 = 수요기업 모집과 동일 패턴
    "수혜기업모집",
    "육성과정",   # 교육 프로그램, 조달 대상 아님
    "시찰단",     # 해외 전시회 시찰단 모집, 수주 대상 아님
    "컨설팅 지원",  # 컨설팅 수혜기업 모집 = 우리가 수주할 사업이 아님
    "컨설팅지원",
    "컨설팅 모집",  # 컨설팅 수혜기업 모집 패턴
    "컨설팅모집",
    "진단 컨설팅",  # 산업보안 진단 컨설팅 등
    "진단컨설팅",
    "무료 교육",   # 교육 프로그램 참가자 모집
    "무료교육",
    "전문가 양성",  # 인력양성 프로그램
    "전문가양성",
    "전문가 과정",  # 전문가 교육과정 — AX 전략 전문가 과정 등
    "전문가과정",
    "참가자 모집",  # 교육/세미나/전시회 참가자 모집
    "참가자모집",
    "참관객 모집",
    "수강생 모집",
    "수강생모집",
    ...
    "교육생 모집",  # 교육 프로그램 모집
    "교육생모집",
    "ksp",         # Knowledge Sharing Program — 국제협력 컨설팅
    "민간 제안사업",  # 정책 제안 공모
    "민간제안사업",
    "해외봉사",
    "국제협력",
    "챔피언십",    # 경진대회/행사 — WP-AX 챔피언십 등
    "innovation day",  # 행사/포럼
    "지식재산평가",  # 특허 컨설팅 — 제조AI와 무관
    "지식재산 평가",
    "ip투자연계",  # IP 투자 평가 — 제조AI와 무관
    "공유오피스",   # 사무실 입주 모집
    "공유 오피스",
    "가상융합",    # VR/AR 콘텐츠 — 제조AI 무관
    "실감콘텐츠",  # 실감형 콘텐츠 제작
    "실감형 콘텐츠",
    "실감 콘텐츠",
    "영상 공모전",  # 영상/아이디어 공모전
